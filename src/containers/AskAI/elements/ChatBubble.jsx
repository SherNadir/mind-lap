...
const ChatBubble = ({ message }) => {
  ...
  const bubbleClass = isSender
    ? "bg-primary text-blue-800 ml-auto border-transparent border rounded-br-lg "
    : "bg-primary-200 text-grey-800 mr-auto border-grey-100 border rounded-bl-lg ";
  ...
  return (
    <div className={`flex ${initialsClass}`}>
      ...
      >
        <div
          className={`w-8 h-8 min-w-[2rem] min-h-[2rem] max-w-[2rem] max-h-[2rem] rounded-full flex items-center justify-center bg-white-300  text-grey-800 text-xs relative  `}
        >
          {userProfile}
        </div>
        <div className={`${bubbleClass} relative  rounded-2xl `}>
          ...
          >
            ...
            >
              <p
                className={`text-sm text-white font-normal w-full ${
                  hasAttachment ? "text-end" : "text-start"
                }`}
              >
                {message.text}
              </p>
              <p className="text-sm text-white font-medium text-right min-w-max self-end flex items-center gap-1">
